...
with mlflow.start_run(run_name=run_name):
    ...
    print("Генерация и сохранение графиков Optuna...")

    # 1. График важности гиперпараметров
    fig_importance = plot_param_importances(study)
    fig_importance.write_html("importance.html")
    mlflow.log_artifact("importance.html", "optuna_plots")

    # 2. График истории оптимизации
    fig_history = plot_optimization_history(study)
    fig_history.write_html("history.html")
    mlflow.log_artifact("history.html", "optuna_plots")

    # 3. График параллельных координат
    fig_parallel = plot_parallel_coordinate(study)
    fig_parallel.write_html("parallel.html")
    mlflow.log_artifact("parallel.html", "optuna_plots")

    # Удаляем временные файлы с диска, чтобы не мусорить в папке с кодом
    os.remove("importance.html")
    os.remove("history.html")
    os.remove("parallel.html")

    print("Графики сохранены в MLflow!")
